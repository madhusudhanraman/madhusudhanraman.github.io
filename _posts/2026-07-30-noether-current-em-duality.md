...
\end{equation}
We can read off from this the symplectic potential, which for duality
rotations is
\begin{equation}
\Theta _ {D} = - Z \wedge \star F \ .
\end{equation}

On the other hand, the variation of the action in
\eqref{eq:maxwell-lagrangian} for electric-magnetic duality rotations
in \eqref{eq:em-duality-rotations}  yields
\begin{equation}
...
\end{equation}
Assuming that $ G \wedge G = 0 $, which is harmless as it is certainly
true on-shell by assumption, we read off
\begin{equation}
B _ {\text{D}} = \frac{1}{2} \left[ A \wedge F - Z \wedge \text{d} Z \right] \ ,
\end{equation}
and so the duality current $ J _ {\text{D}} = \Theta _ {\text{D}} - B _ {\text{D}}  $ is
\begin{equation}
